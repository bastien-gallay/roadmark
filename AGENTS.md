...
`[main]`, which meant a PR based on another branch got no CI at all —
and stacking is what this file tells you to do when work has run in
parallel, so the one shape the repo prescribes was the one shape nobody
checked (#62 merged that way, verified by hand). Don't re-pin it.
