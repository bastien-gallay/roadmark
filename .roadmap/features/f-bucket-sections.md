...
The section carries the bucket, so the bucket column drops out inside it — the same rule that drops a column no feature holds. An undeclared target is the exception: no heading can carry it, so its cell stays rather than being swallowed. Empty buckets emit no heading, untargeted features collect in a trailing section, and `## Details` stays flat and stays one list because it is anchor-addressed and the catalog links into it.
